...
boolean EepromUtil::eeprom_update_bytes(int startAddr, const byte* array, int numBytes) {
	// counter
	int i;
	...
	if (!eeprom_is_addr_ok(startAddr) || !eeprom_is_addr_ok(startAddr + numBytes)) {
		...
		DEBUG_PRINT("address out of range ");
		//Serial.println(startAddr+numBytes);
		DEBUG_PRINTLN(startAddr+numBytes);
		return false;
	}

	for (i = 0; i < numBytes; i++) {
		if(array[i]!=EEPROM.read(startAddr + i)) {
			EEPROM.write(startAddr + i, array[i]);
			//Serial.print(array[i]);Serial.print("+");
			//DEBUG_PRINT(array[i]);DEBUG_PRINT("+");
			//} else {
		}
		//Serial.print(startAddr + i);Serial.print("=");Serial.print(array[i], HEX);Serial.print(' ');
		//DEBUG_PRINT(startAddr + i);DEBUG_PRINT("=");DEBUG_PRINT(array[i], HEX);DEBUG_PRINT(' ');
